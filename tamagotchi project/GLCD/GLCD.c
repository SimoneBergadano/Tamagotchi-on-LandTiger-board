...
void LCD_DrawFullCircle(uint16_t Xpos,uint16_t Ypos, uint16_t radius, uint16_t color){
	...
	for (i = -r; i < +r; i++) {
		for (j = -r; j < +r; j++) {
			...
			if( (i*i)+(j*j) <= (r*r) ) LCD_SetPoint(i+Xpos, j+Ypos, color);
...
void LCD_DrawEmptyRectangle (uint16_t X_start, uint16_t Y_start, uint16_t width, uint16_t height, uint16_t color)
{
